...
describe.skipIf(!process.env.DATABASE_URL)('gateway idempotency (#8 replay-first)', () => {
  ...
  it('double-submit of the same commit applies once and replays the ORIGINAL outcome', async () => {
    ...
    // F2.2 erratum 2: facts replay verbatim — ledgerId included — except
    // disposition, which marks the replay. Spec 2026-07-20 §3: presentation
    // is STRIPPED (data._uiAction dropped, card-directive _message replaced).
    const { data: freshData, ...freshRest } = r1
    const { data: replayData, ...replayRest } = r2
    ...
    expect(rd.candidateProductId).toBe(fd.candidateProductId) // facts still equal
    expect(rd._confirmation).toEqual(fd._confirmation)        // idempotent ✓ line re-renders
  ...
  it('repeatable commit: collect_customer_field same field + same value → replay with the original envelope', async () => {
    ...
    // F2.2 erratum 2: facts verbatim + disposition marker; spec 2026-07-20 §3:
    // presentation stripped on the replay.
    const { data: freshData, ...freshRest } = r1
    const { data: replayData, ...replayRest } = r2
    expect(replayRest).toEqual({ ...freshRest, disposition: 'replay' })
    const fd = freshData as Record<string, unknown>
    const rd = replayData as Record<string, unknown>
    expect(rd.fieldSaved).toBe(fd.fieldSaved) // facts still equal
    // spec 2026-07-20 §3: replay strips presentation
    expect(rd._uiAction).toBeUndefined()
    expect(rd._message).toBe(REPLAY_NOTICE)
