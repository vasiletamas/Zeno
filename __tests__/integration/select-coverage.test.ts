...
it('writes Application columns only — no Answer rows (single writer, T5.D2)', async () => {
  ...
  const r = await executeCommit({ tool: 'select_coverage', args: { tier: 'standard', level: 'level_1' }, actor: 'gui', customerId: c.id, conversationId: conv.id, toolContext: ctx(c.id, conv.id) })
...
it('invalid level for tier → rejected(invalid_level_for_tier); re-invocation with a DRAFT quote → re_rating + quote expired', async () => {
  const { c, conv } = await openApp()
  const bad = await executeCommit({ tool: 'select_coverage', args: { tier: 'standard', level: 'no_such' }, actor: 'agent', customerId: c.id, conversationId: conv.id, toolContext: ctx(c.id, conv.id) })
  expect(bad).toMatchObject({ outcome: 'rejected', reason: 'invalid_level_for_tier' })
  await executeCommit({ tool: 'select_coverage', args: { tier: 'standard', level: 'level_1' }, actor: 'agent', customerId: c.id, conversationId: conv.id, toolContext: ctx(c.id, conv.id) })
  const app = await prisma.application.findFirstOrThrow({ where: { customerId: c.id } })
...
it('addon toggle carries cascade_expand / questions_removed (#4); legacy mutators are gone', async () => {
  const { c, conv } = await openApp()
  const on = await executeCommit({ tool: 'select_coverage', args: { tier: 'standard', level: 'level_1', addon: true }, actor: 'agent', customerId: c.id, conversationId: conv.id, toolContext: ctx(c.id, conv.id) })
