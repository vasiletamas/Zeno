...
describe('T13.D8 hybrid state surface', () => {
  ...
  it('flagsForReview is empty on a calm snapshot and rendered in the injected summary only when present', () => {
    ...
    const flagged = deriveAndExpose(makeSnapshot({ dnt: { signed: true, valid: true, validUntil: soon, coversProductTypes: ['LIFE'], answeredCount: 5, totalCount: 5, sessionActive: false, latest: null, activeSessionId: null, sessionType: null, sessionAnswered: 0, sessionTotal: 0 } }))
