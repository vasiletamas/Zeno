...
describe('T13.D8 hybrid state surface', () => {
  ...
  it('flagsForReview surfaces alert-worthy facts (expiring DNT, conflict fields) in DerivedStateV3', () => {
    ...
    const r = deriveAndExpose(makeSnapshot({
      dnt: { signed: true, valid: true, validUntil: soon, coversProductTypes: ['LIFE'], answeredCount: 5, totalCount: 5, sessionActive: false },
  ...
  it('flagsForReview is empty on a calm snapshot and rendered in the injected summary only when present', () => {
    ...
    const flagged = deriveAndExpose(makeSnapshot({ dnt: { signed: true, valid: true, validUntil: soon, coversProductTypes: ['LIFE'], answeredCount: 5, totalCount: 5, sessionActive: false } }))
