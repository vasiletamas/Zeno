...
vi.mock('@/lib/db', () => ({ prisma: { product: { findMany: (...a: unknown[]) => productFindManySpy(...a) } } }))
...
describe('compareProducts', () => {
  ...
  it('does NOT leak per-level premiumAnnual but keeps product premiumRange', async () => {
    ...
    for (const prod of comparison) {
      expect(prod.premiumRange).toBeTruthy() // product-level range stays
    ...
    // No exact per-level-only price (350) appears anywhere in the serialized result
