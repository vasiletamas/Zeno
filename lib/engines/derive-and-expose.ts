...
export const ACTION_RULES: ActionRule[] = [
  ...
  // T8 (design 2026-07-15 §1): purchase intent is capturable the MOMENT the
  // customer commits — every selling phase, DISCOVERY through PAYMENT.
  // POLICY is post-sale (no selling), so the commit simply isn't exposed
...
export function deriveAndExpose(s: DomainSnapshot, config?: { identityRequirements?: IdentityRequirementsTable }): DeriveAndExposeResult {
  ...
  const state: DerivedStateV3 = {
    ...
    intent: s.intent ?? null, // T8: verbatim slice — briefing + get_current_state read one source
  }
