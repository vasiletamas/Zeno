...
export interface DomainSnapshot {
  ...
  application: { id: string; status: 'OPEN' | 'PAUSED' | 'REFERRED' | 'COMPLETED' | 'CANCELLED'; tier: string | null; level: string | null; addon: boolean | null; answeredCount: number; requiredCount: number; missingCodes: string[]; frozen: boolean; createdAt?: string; medicalDeclarations?: import('./medical-declarations').MedicalDeclarationFacts } | null // frozen (D1, T7.D1): frozenAt set OR a Quote row exists in ANY state — post-quote mutation is engine-illegal // full T5.D6 set (B4); the loader nulls CANCELLED pointers; createdAt (E4.2) feeds open-item age; medicalDeclarations (T6.D3 deviation 2026-07-06) optional — undefined ≡ none required, keeps pre-existing fixtures compiling
  /**
  ...
  /**
   * P2-8: did the CUSTOMER speak (a role='user' Message) AFTER the live quote
   * was issued? cancel_quote is gated on this so the model cannot silently
   * self-cancel a fresh quote and loop set_application. Optional — undefined ≡
   * false — keeps pre-existing snapshot literals compiling.
   */
  customerMessagedSinceQuote?: boolean
