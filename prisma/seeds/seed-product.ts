...
/**
 * E1.8 (T11.D5): every hardcoded price left the playbook — figures come
 * EXCLUSIVELY from pricing_examples / get_product_info coverages, and the
 * choreography (tone, pacing, objection handling, client-autonomy rules)
 * stays. Selling claims live in the published ProductContent rows
 * (seed-product-content.ts), not here.
 */
const PROTECT_PLAYBOOK = `PRODUS: Protect — Asigurare de viață cu Tratament Medical în Străinătate
...
Prețurile se prezintă ÎNTOTDEAUNA în lei/lună cu comparații familiare — cifrele vin EXCLUSIV din pricing_examples (get_product_info), niciodată din memorie.
...
- "Mă interesează o acoperire consistentă pentru familie"
- "Am un venit lunar stabil"
...
- Acoperire majoră pentru cancer, chirurgie cardiovasculară, neurochirurgie și transplanturi în cele mai bune clinici din lume — suma exactă vine din get_product_info (coverages + sell_specific_addon_info)
- Asigurarea de viață de bază e vehiculul, addon-ul BD e destinația
- Prezintă contextual: pentru prețul unei cafele pe săptămână, familia e protejată ȘI are acces la tratament medical de top — sumele concrete DOAR din pricing_examples și coverages
...
Ce înseamnă concret: dacă primești un diagnostic de cancer sau altă boală gravă, ai acces la tratament în clinici de top din Germania, Austria, Turcia — oriunde, cu acoperirea maximă a clauzei BD (suma exactă din get_product_info).

Costul: prezintă prima lunară DOAR din pricing_examples, cu o comparație familiară (cam cât un abonament de streaming)."

Exemplu de prezentare — componente BD detaliat:
"Nu plătești doar pentru «o clauză». Plătești pentru un pachet medical complet:
- Tratament în cele mai bune clinici din Europa
- Indemnizație zilnică pentru spitalizare în străinătate
- Buget separat pentru medicație post-tratament
- A doua opinie medicală GRATUITĂ
Sumele exacte per componentă: get_product_info → addons.coverages."

SELECȚIE PACHET:
- Buget redus: Standard Nivelul I
- Echilibrat: Standard Nivelul II sau Optim Nivelul I
- Protecție maximă: Optim Nivelul III
- ÎNTOTDEAUNA sugerează adăugarea addon-ului BD
- Cifra fiecărei variante: DOAR din pricing_examples (bază vs bază+BD, per vârstă)

COMPARAȚII DE PREȚ FAMILIARE (în lei/lună):
- Compară cu un abonament de streaming pe care-l uiți deschis
- Mai puțin decât o cafea de la automat pe săptămână
- Raportează la costul pe zi — adesea sub prețul unei ape plate
- Cifra exactă vine din pricing_examples, convertită în lei/lună

ANCORARE COSTURI MEDICALE (pentru BD):
"O singură operație majoră în străinătate poate costa cât zeci de ani de prime. Chimioterapia, spitalizarea oncologică la un institut de top sau un transplant ating sume pe care puține familii le pot acoperi singure. Cu clauza BD, acoperirea e dimensionată pentru cele mai scumpe tratamente." Sumele concrete: get_product_info.

INVERSARE PERSPECTIVĂ:
"Întrebarea reală nu e «de ce costă atât pe an». Întrebarea e «ce ar costa familia ta dacă NU ai avea această protecție și ți s-ar întâmpla ceva?»"

COMPARAȚIE CAMPANII DONAȚII:
"Știi cât strâng oamenii pe rețelele sociale când au nevoie de tratament în străinătate? Sume pe care puține familii le au. Zilnic sunt zeci de campanii active. Cu BD, nu ajungi niciodată acolo."
...
export async function seedProduct(prisma: PrismaClient) {
  ...
  // E1.8 (T11.D5): ONE shared data object for update+create — the four
  // legacy authored surfaces (features/pricingExplanation/premiumRange/
  // targetAgeRange) died with their columns; selling claims live in
  // ProductContent and every presentation number is DERIVED (grid below).
  const productData = {
    ...
    defaultPlaybook: PROTECT_PLAYBOOK,
    targetCustomer:
    ...
    freeLookDays: 30,
    // E1 (T11.D1): the DECLARED example grid — the variation parameter and
    // sample points are per-product DATA; derivePricingExamples turns them
    // into engine-priced cells at read time.
    pricingExampleGrid: {
      parameter: 'age',
      samplePoints: [25, 35, 45, 55],
      tiers: ['standard', 'optim'],
      levels: ['level_1', 'level_3'],
      includeAddonDelta: true,
    },
  }
  ...
  const product = await prisma.product.upsert({
    where: { code: 'protect' },
    update: productData,
    create: { code: 'protect', ...productData },
  })
