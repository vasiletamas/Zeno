...
10. **"LLM retry detected" anomaly is 100% false positive — FIXED 2026-07-07.** The call-count heuristic is retired; the registry emits real `llm:call:retry` (at the actual backoff site, with attempt/delay/error class) and `llm:failover` (incl. circuit-open skips) events, traceId-correlated by the gateway; the anomaly subscriber maps them info/warning. Tests: `llm-retry-anomaly.test.ts`.
11. **`requires_confirmation` failures are invisible to diagnostics — CLOSED (already fixed).** The `confirmation_stalled` ratchet check (warn at 2, error at ≥3 trailing reissues for a tool with no subsequent apply) is exactly the prescribed escalation; verified in `checks-basic.ts`.
12. **Silent turn failures / dead air — FIXED 2026-07-07.** The orchestrator pipeline runs inside an abort guard: a fatal error records a critical `turn_aborted` anomaly, persists the TurnDebug row from the events collected so far (awaited), emits `turn:end`, and yields a structured retryable error; the GUI error bubble gained a Retry button that resends the failed message (`retryLastMessage`). Investigation note: snapshot-loader and deriveAndExpose failures were already absorbed by fallbacks — the guard covers the remaining unhandled class (proven via `getAgentConfig`, which runs outside any local catch). Test: `turn-abort-debug.test.ts`.
13. **Turn latency — DECISION RECORDED 2026-07-07 (advisory, not a defect).** No code change: latency is dominated by two LLM rounds on GPT-5.4; the candidate levers (fast-path gate expansion, a smaller tool-round model, streaming-first UX) each trade answer quality or complexity and need product sign-off + measurement. Revisit with the cost dashboards that P1-9 just unblocked (real token/latency data now records).

### P2 — test integrity / quality

14. **Vacuous sim passes — FIXED 2026-07-07.** `goalReached` is now a hard PASS criterion (a vacuous run FAILS with `goal_not_reached`); it immediately caught dnt-refusal running out of turns before the signing ask (budget raised 20→40) AND a harness defect: the auto-clicked confirm cards were signing the DNT the scripted customer had just refused (`replayConfirms: false` for refusal scenarios). All three fixtures re-recorded from genuinely goal-reaching runs.
15. **Tool schema/description inconsistency — FIXED 2026-07-07 (decision: cards own ALL confirm round-trips; the model never resends).** The four "re-call with the token" descriptions rewritten to card-only language; `sign_medical_declarations` lost its model-facing confirmToken param and gained the zod schema it was missing (it rode the permissive unknown-tool fallback). **Bonus finding of the P0-6 class:** `cancel_quote`, `cancel_application`, `change_payment_option`, `request_cancellation` returned `requires_confirmation` with NO card in `CONFIRMABLE_TOOLS` — their tokens had no carrier in the real GUI. All four gained cards + adapter round-trips (material args ride along). Tests: confirm-required + registry-kind pins.
16. **UX polish — DECISION RECORDED 2026-07-07 (advisory).** Raw enum tokens and repeat-phrasing under stalls are prompt/product-tuning work, already partially mitigated (the `repeated_assistant_message` checker monitors it; the P0-7 questionnaireContext surface gives the agent labeled options to phrase from). Track via sim transcripts; no deterministic fix is appropriate.
