...
export function pickAnswer(msg: string, policy: SpecSimScenario['answerPolicy']): string {
  ...
  // channel verification never started (run cmr9cq7e5 msg 69).
  if (/adresa (ta )?de email/.test(m)) return 'ion.sim@example.com'
  ...
  if (/data na[șs]terii|aaaa-ll-zz/.test(m)) return '1996-02-29' // MUST match the CNP above (a conflict downgrades identity provenance)
  if (/num[ăa]r(ul)?( t[ăa]u)? de telefon/.test(m)) return '0722334455' // KYC field — the decomposed needs (engine 1.35.0) make the agent ask for it
